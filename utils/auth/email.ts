...
export async function sendCompanyInvitation(
  ...
  try {
    ...
    const msg = {
      to: recipientEmail,
      from: process.env.SENDGRID_FROM_EMAIL || 'noreply@example.com', // Use environment variable or fallback
    ...
    console.log(`Invitation email sent to ${recipientEmail}, status: ${response[0].statusCode}`)
...
export async function sendWelcomeEmail(
  ...
  try {
    ...
    // Log response status (for debugging)
    console.log(`Welcome email sent to ${userEmail}, status: ${response[0].statusCode}`)
